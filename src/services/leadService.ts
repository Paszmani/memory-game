 ...
 * fixos (terminal, tema, score, timestamp). No Electron, o preload expõe
...
export async function saveLead(lead: Lead): Promise<void> {
  ...
  all.push(lead);
  await setJson(STORAGE_KEYS.leads, all);
}
...
export async function getStoredLeads(): Promise<Lead[]> {
  const parsed = await getJson<unknown>(STORAGE_KEYS.leads, []);
  return Array.isArray(parsed) ? (parsed as Lead[]) : [];
}

export async function clearStoredLeads(): Promise<void> {
  await removeItem(STORAGE_KEYS.leads);
}
...
function csvFileName(): string {
  ...
  return `leads-memoria-${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}.csv`;
}
